...
import { ShieldCheck, Eye, Database, Lock, Trash2, RefreshCcw, AlertTriangle, Server, Globe, CheckCircle2, MousePointerClick, Copy, Info } from 'lucide-react';
import './styles.css';
  ...
  return <main>
    <section className="hero">
      ...
      <div className="heroActions">
        ...
        <button className="secondary" onClick={copyUrl}><Copy size={18}/> Copy site URL</button>
      </div>
    </section>
    ...
    <section className="card" id="demo">
      ...
      <form onSubmit={saveName} className="form">
        ...
        <label className="consent"><input type="checkbox" checked={consent} onChange={(e) => setConsent(e.target.checked)} /> I understand this is an educational demo and my chosen name will be stored temporarily for 24 hours.</label>
        <button className="primary button" disabled={loading}>{loading ? 'Working...' : 'Save demo identity'}</button>
      </form>
      <div className="status">{message}</div>
    </section>

    {found && record && <section className="result">
      <div>
    ...
    <section className="plainBox dark">
